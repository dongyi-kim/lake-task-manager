...
### ★ 계약 배터리의 천장 — `tools/agent_user_review.py` 를 만든 이유

> 사용자: "직접 답변 품질을 검토하라고 한 게 몇 번째인데 이런 것들이 필터링이 안 되고
> **내 테스트에서** 발견되는지 모르겠다. user agent 를 만들어서 진짜 사람 이용자의
> 관점에서 평가해야 한다."

정확한 진단이다. 41/41 이 초록인 채로 아래가 전부 통과했다 — **전부 사용자가 먼저 봤다**:

| 통과한 결함 | 계약이 못 본 이유 |
|---|---|
| Sub-Task 이름이 "설계 단계/구현 단계/검증 단계" | 계약은 **자식이 3개 이상인가**를 셌다. 제목의 쓸모는 안 봤다 |
| 미리보기 상위가 `DL-102` 뿐 | 계약은 API 응답만 본다. **화면 조립은 계약 밖**이다 |
| 코멘트 요청에 "완료 조건이 무엇인가요" | CMTB1 은 `change_plan` 만 봤다 — **질문은 안 봤다** |
| 참조가 평문으로 나옴 | 마크다운 렌더러는 파이썬 배터리가 지나는 자리가 아니다 |

원인은 하나다. **계약에는 내가 이미 예상한 실패만 들어 있다.** 케이스를 쓰는 순간 나는
"무엇이 틀릴 수 있나"를 정하고, 그 밖은 영원히 안 본다. 케이스를 늘려도 이 천장은 안
올라간다 — 늘리는 사람이 같기 때문이다.

그래서 평가자를 **계약 없이** 세웠다. 네 가지가 다르다:

1. **계약을 안 준다** — 케이스가 무엇을 재려 했는지 모른 채 결과만 본다. 계약을 주면
   계약대로만 본다
2. **산출물 전체**를 준다 — 답변 문장 + 초안 항목·Sub-Task 제목·카드 필드·질문 보기.
   "설계 단계"도 "상위 DL-102"도 답변 문장에는 없고 **카드에 있다**
3. **사실 대조·렌더 진단을 코드가 계산해** 함께 준다 — 텍스트만 읽어서는 "이 참조가
   진짜인가"(축③)와 "화면에서 깨지는가"(축⑤)를 판정할 수 없다. `grounding.check` +
   언급된 키의 실제 제목·상태, 링크 안 걸린 참조·열 어긋난 표·고아 `[N]` 마커
4. **점수를 안 매긴다. 불평을 받는다** — 3.5든 4.2든 다음 행동을 안 바꾼다. 불평은 고칠
   자리를 바로 가리킨다

페르소나는 **둘**이다(PMO=승인해도 되나 / 실무=오늘 당장 뭘 하나). 그리고 사용자가 지금까지
지적한 것을 **재발 감시 목록**으로 싣는다 — 같은 지적을 두 번 받는 것이 가장 나쁘다.

**첫 실행에서 41/41 초록인 상태로 blocker 7건.** 그중 최악은 **내 회귀**였다: 그날 넣은
인터뷰 슬롯(배경·완료조건·분할)이 `DRAFTS_TICKETS` 에 `MODIFY` 가 들어 있는 탓에
**코멘트만 남기는 요청까지** 샜다. 계약은 초록, 사람은 즉시 불평.

> **★ 계약은 "내가 아는 실패"의 회귀를 막는다. 계약 없는 평가자는 "내가 모르는 실패"를
> 연다.** 둘은 대체재가 아니다 — 새 결함은 평가자가 열고, 고친 뒤 계약에 못 박는다.

## 6. 현재 상태 (지속 루프 Round A~U + 정합 감사 + 품질 라운드 C·D·E 후)

**배터리 현황 — 전부 "한 번의 실행"에서 잰 값이다**(여러 번 돌려 좋은 회차를 고르지 않는다).

| 배터리 | 무엇을 재나 | 결과 |
|---|---|---|
| `pytest` | 코드 보장 | **1056 passed** |
| `tools/agent_scenarios.py` 41 (한 프로세스 한 실행) | 조사·답변 (계약 + 6축) | **41/41** · 평균 3.63 |
| `tools/agent_create_suite.py` 20 | 초안 구조 + 본문 4섹션 | **20/20** (기준선 16/20) |
| `tools/agent_user_review.py` 8흐름 | **계약 없이** 사람 관점 불평 | 상시 열려 있음(0건이 목표가 아니다) |

judge 점수는 **게이트가 아니다**(§5-f). 통과 판정은 계약(`EXPECT`)이 하고, 점수는
`⚑읽어볼 것` 표시로만 쓴다 — 정성 판정은 클로드가 출력을 직접 읽어서 한다.

**세 배터리의 자리가 다르다.** pytest 는 코드가 지키는 것, 시나리오·생성 스위트는 **아는
실패**의 회귀, 사용자 평가자는 **모르는 실패**의 발굴(§5-g). 앞의 셋은 초록이 목표지만
평가자는 그렇지 않다 — 불평 0건은 "다 됐다"가 아니라 **평가자가 무뎌졌다**는 신호로 읽는다.
...
16-b. **`REPORT_BUG` enum 걷어내기** (사용자 지적 2026-08-10) — 이 갈래는 `PLAN_WORK` 와
    **지나는 노드도 도구도 같다.** 코드 전체에서 다르게 쓰이는 곳이 Refiner 의 goal 하나
    뿐이었다 — "Task 를 만드는데 type 이 Bug" 인 것이라 **갈래가 아니라 산출물 유형**이다.
    Intent 주석은 "노드가 달라서 나눴다"고 적혀 있었는데 MY_DAY 에는 맞고 이쪽은 사실이
    아니었다(주석을 고쳤다).
    **해는 이미 없앴다** — 본문 규율을 의도가 아니라 **요청의 낱말**로 고르게 해서, 분류가
    흔들려도 재현·기대·실제가 유지된다. 남은 것은 enum 자체를 걷어내는 일이다:
    ①planner 스키마·few-shot 에서 제거 ②`NEEDS_RESEARCH`/`DRAFTS_TICKETS` 정리
    ③`session._plan_for` 확인. 지금 급하지 않은 이유는 **동작이 이미 안전해졌기** 때문이다 —
    남은 것은 분류 부담과 코드 군더더기다.
...
- **★ 새 갈래를 만들면 그 갈래의 *모든 지점*을 재라** — 구조 합의 단계 하나를 넣었더니
  후속 결함이 **다섯** 나왔다: 게이트가 넓어 단순 요청까지 걸림(20/20→16/20) · 사용자가
  이미 말한 형태를 되물음 · Responder 분기 순서 때문에 구조도가 **영원히** 안 그려짐 ·
  피드백이 `modify` 로 샘 · **합의 뒤 턴**에 본문을 첫 항목만 채움. 전부 "새 턴을 하나
  만들면 그 앞뒤 턴이 전부 새 경우가 된다"는 한 가지다. 새 갈래를 넣은 커밋에서는
  **관련 배터리를 전부** 다시 돌린다 — 새 케이스만 보면 옛 케이스가 조용히 깨진다.
- **PowerShell 파이프(`| Select-String`)는 끝까지 버퍼링한다** — 긴 실행의 진행 상황이
  파일에 하나도 안 쌓인다. 진행을 보려면 원시 출력을 파일로 흘리고 그 파일을 `grep` 한다.
