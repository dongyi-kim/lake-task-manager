...
§5-e) → **품질 라운드 D**(조사·답변, §5-f) 완료. **1019 tests green**,
**생성 스위트 20/20 한 실행 전수 통과**(기준선 16/20), 시나리오 배터리 **한 실행 27/29**
(남은 둘은 케이스 결함이 아니라 **배터리의 색인 누적** — §5-f).
...
| 하네스 | 명령 | 용도 · 비용 |
|---|---|---|
| 전체 pytest | `python -m pytest tests/ -q` | **fake LLM + mock** — 그래프 분기·가드·도구 계약·회귀. 현재 **1019 passed**, ~1.5분, $0 |
...
검증 계약을 붙였다. 결과는 **워크플로우 14/14 + DATA·PROG 15/15 = 29/29, 각각 단일 실행**.
...
아니다 — 멀티턴 케이스는 자기 턴 사이의 누적이 필요하다). 26/29 → **27/29** 로 올랐고
남은 둘(DATA4·DATA11)도 작은 배치에서는 통과하므로 **누적이 아직 남아 있다**.
...
| 배터리 | 무엇을 재나 | 결과 |
|---|---|---|
| `pytest` | 코드 보장 | **1019 passed** |
| `tools/agent_scenarios.py` 29 (한 실행) | 조사·답변 (계약 + 6축) | **27/29** · 평균 3.72 — 남은 둘은 색인 누적(§5-f) |
...
- **1019 pytest green**. 지속 시나리오 루프(probe 러너 `tools/agent_probe.py`·브라우저
