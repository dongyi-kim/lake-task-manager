...
**지속 시나리오 루프(Round E)에서 확인된 미해결 갭 — 다음 세션 최우선:**
- **조건 일괄 수정 미지원**: "ETL 마감 지난 티켓 전부 P1으로" — ①조사가 대상을 못 찾음
  (JQL 로 찾으면 5건 실재) ②change_plan 이 단건 전용이라 여러 키 일괄 변경 카드가 없다.
  update_tickets(bulk) 도구는 있으니 refiner change 복수화+카드/지문 확장이 필요 (E2 실측)
- **온보딩/소개 질의**: knowledge/03 모듈 정의·Epic 목록(find_parent_epic)을 안 쓰고
  "Epic 정의 없음" 오답 — 소개류 사전취합(모듈 정의+Epic 목록 주입) 필요 (E4 실측)
- **modify 답변의 제목 날조**: 변경 카드 응답에서 실제 제목 대신 요청 문구를 제목처럼
  표기("DL-9062 '리샘플링 기준 논의'") — grounding wrong_titles 가 왜 안 잡았는지 조사 (E1 실측)
