...
def _authorize_network(model: str, simple_model: str, timeout: float) -> None:
    ...
    try:
        routed_provider, _identity = _provider_context()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    try:
        _authorize_network(model, simple_model, timeout)
    except EvaluationPreflightError as exc:
        print(str(exc), file=sys.stderr)
        return 3

    # This marker is intentionally created here, after the endpoint check, and inherited only
    # by the selected child process. Direct runner invocations never manufacture it themselves.
