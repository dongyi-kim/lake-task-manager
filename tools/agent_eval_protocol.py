...
CANDIDATE_INPUTS = (
    ...
    ROOT / "requirements-sso.txt",
    ROOT / "requirements-test.txt",
)
EVALUATION_HARNESS_INPUTS = tuple(
    ROOT / "tools" / name for name in (
        "agent_eval_protocol.py",
        "agent_eval_isolation.py",
        "agent_eval_launcher.py",
        "agent_scenario_eval.py",
        "agent_eval_contracts.py",
        "agent_eval_claims.py",
        "agent_lang_ab.py",
...
@lru_cache(maxsize=1)
def candidate_tree_sha256() -> str:
    """Hash the behavior-bearing candidate tree, including untracked source files.

    A commit plus a dirty boolean cannot distinguish two working-tree candidates.  The
    battery manifest already owns evaluator/checker identity, so this digest is intentionally
    limited to runtime code, configuration, prompts/assets, and direct dependency manifests.
    Ignored caches and generated bytecode never participate.
    """
    digest = hashlib.sha256()
    files: list[Path] = []
...
@lru_cache(maxsize=1)
def evaluation_harness_tree_sha256() -> str:
    """Fingerprint the runner and isolation code omitted from the candidate tree."""
    digest = hashlib.sha256()
    for path in sorted(EVALUATION_HARNESS_INPUTS, key=lambda item: item.as_posix()):
        relative = path.relative_to(ROOT).as_posix().encode("utf-8")
        digest.update(len(relative).to_bytes(4, "big"))
        digest.update(relative)
        body = path.read_bytes()
...
@lru_cache(maxsize=1)
def runtime_package_set_sha256() -> str:
    """Fingerprint the resolved Python environment without exposing install paths."""
    packages = sorted({
        (str(dist.metadata.get("Name") or "").strip().casefold(), str(dist.version))
        for dist in importlib.metadata.distributions()
        if str(dist.metadata.get("Name") or "").strip()
    })
    return _json_hash([f"{name}=={version}" for name, version in packages])
...
def _git(*args: str) -> str:
    available, output = _git_checked(*args)
    return output if available else ""


def git_snapshot() -> dict[str, Any]:
    tracked_available, tracked_status = _git_checked(
        "status", "--porcelain", "--untracked-files=no",
    )
    full_available, full_status = _git_checked(
        "status", "--porcelain", "--untracked-files=all",
    )
    commit_available, commit = _git_checked("rev-parse", "HEAD")
    status_available = tracked_available and full_available
    dirty_untracked = any(
        line.startswith("?? ") for line in full_status.splitlines()
    )
    return {
        "commit": commit if commit_available and commit else "unknown",
        "gitStatusAvailable": status_available,
        "dirtyTrackedFiles": bool(tracked_status),
        "dirtyUntrackedFiles": dirty_untracked,
...
def build_run_metadata(
    *,
    ...
    harness_hash = evaluation_harness_tree_sha256()
    git = git_snapshot()
    ...
    provider_endpoint_identity = str(
        os.getenv("LTM_EVAL_EFFECTIVE_PROVIDER_IDENTITY_SHA256") or ""
    ).strip().lower()
    if not re.fullmatch(r"[0-9a-f]{64}", provider_endpoint_identity):
        provider_endpoint_identity = "unrecorded"
    ...
    benchmark = {
        ...
        "evaluationHarnessTreeSha256": harness_hash,
    }
    structured_backend = str(
        os.getenv("LTM_AGENT_STRUCTURED_OUTPUT_BACKEND") or "instructor"
    ).strip().lower()
    structured_fallback_policy = str(
        os.getenv("LTM_AGENT_STRUCTURED_OUTPUT_FALLBACK") or "allow"
    ).strip().lower()
    structured_backend_version = (
        _installed_version("instructor")
        if structured_backend == "instructor" else "legacy-internal"
    )
    package_set_hash = runtime_package_set_sha256()
    candidate = {
        "model": model,
        "simpleModel": simple_model,
        "provider": provider,
        "providerEndpointIdentitySha256": provider_endpoint_identity,
        "structuredOutputBackend": structured_backend,
        "structuredBackendVersion": structured_backend_version,
        "structuredBackendFallbackPolicy": structured_fallback_policy,
        "promptVersion": prompt_version,
        "candidateCommit": git["commit"],
        "gitStatusAvailable": bool(git.get("gitStatusAvailable", False)),
        "candidateDirty": bool(
            not git.get("gitStatusAvailable", False)
            or git["dirtyTrackedFiles"] or git.get("dirtyUntrackedFiles", False)
        ),
        "candidateTreeSha256": git["candidateTreeSha256"],
        "runtimePackageSetSha256": package_set_hash,
    }
    benchmark_key = _json_hash(benchmark)
    candidate_key = _json_hash(candidate)
    comparability_key = _json_hash({
    ...
    if git.get("dirtyUntrackedFiles", False):
        reasons.append("candidate has untracked working-tree files")
    if not git.get("gitStatusAvailable", False):
        reasons.append("git working-tree status is unavailable")
    ...
    if provider_endpoint_identity == "unrecorded":
        reasons.append("provider endpoint identity is not recorded")
    if structured_fallback_policy != "forbid":
        reasons.append("structured backend fallback policy is not fail-closed")
...
def render_report_standard_block(metadata: Sequence[Mapping[str, Any]]) -> str:
    ...
    rows = [
        (item["battery"]["name"], item["battery"]["batteryVersion"],
         ...
         item["battery"]["specializedReviewSpecSha256"], item["benchmarkKey"],
         item["candidateKey"], item["comparabilityKey"])
    ...
    lines = [
        ...
        f"| evaluationHarnessTreeSha256 | `{run['evaluationHarnessTreeSha256']}` |",
        ...
        "### Battery identity",
        "",
        "| Suite | batteryVersion | batteryManifestSha256 | specializedReviewSpecSha256 | benchmarkKey | candidateKey | comparabilityKey |",
        "|---|---|---|---|---|---|---|",
    ]
    lines.extend(
        f"| {suite} | `{version}` | `{manifest}` | `{specialized}` | `{benchmark}` | "
        f"`{candidate}` | `{comparable}` |"
        for suite, version, manifest, specialized, benchmark, candidate, comparable in rows
    )
    lines.extend([
        "",
        "### Candidate identity",
        "",
        "| Suite | runKind | runGroupId | repeatIndex | candidateOrderIndex | candidateKey | model | simpleModel | provider | providerEndpointIdentitySha256 | structuredOutputBackend | structuredBackendVersion | structuredBackendFallbackPolicy | promptVersion | candidateCommit | gitStatusAvailable | candidateDirty | candidateTreeSha256 | runtimePackageSetSha256 |",
        "|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|",
    ])
    for item in metadata:
        item_run = item["run"]
        lines.append(
            f"| {item['battery']['name']} | `{item_run['runKind']}` | "
            f"`{item_run['runGroupId']}` | `{item_run['repeatIndex']}` | "
            f"`{item_run['candidateOrderIndex']}` | `{item['candidateKey']}` | "
            f"`{item_run['model']}` | "
            f"`{item_run['simpleModel']}` | `{item_run['provider']}` | "
            f"`{item_run['providerEndpointIdentitySha256']}` | "
            f"`{item_run['structuredOutputBackend']}` | "
            f"`{item_run['structuredBackendVersion']}` | "
            f"`{item_run['structuredBackendFallbackPolicy']}` | "
            f"`{item_run['promptVersion']}` | `{item_run['candidateCommit']}` | "
            f"`{item_run['gitStatusAvailable']}` | `{item_run['candidateDirty']}` | "
            f"`{item_run['candidateTreeSha256']}` | "
            f"`{item_run['runtimePackageSetSha256']}` |"
        )
    lines.extend([
        "",
        ...
        "점수를 교체하지 않음. 후보 간 품질·효율 비교는 `benchmarkKey`가 같을 때만 수행하며, "
        "`candidateKey`는 모델/provider/backend/prompt/tree가 다른 후보를 식별한다. "
        "`comparabilityKey`는 동일 benchmark와 동일 candidate의 반복 실행을 식별하는 진단 키다.",
        "",
        "## 실행 격리",
        "",
