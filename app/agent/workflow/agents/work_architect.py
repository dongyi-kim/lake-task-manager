...
from app.agent.workflow.work_architect.apply_pipeline import apply_work_architect
from app.agent.workflow.work_architect.contracts import (
    COMMENT_SCHEMA,
    CREATE_SCHEMA,
    QUESTION,
    SCHEMA,
...
from app.agent.workflow.work_architect.context import (
    current_request_boundary_text as _current_request_boundary_text,
    meeting_request_boundary_text as _meeting_request_boundary_text,
    typed_continuation_contract as _typed_continuation_contract,
)
from app.agent.workflow.work_architect.body_text import (
    base_title as _base_title,
    display_base_title as _display_base_title,
    draft_full_text,
    drop_empty_sections as _drop_empty_sections,
    drop_subtask_ticket_refs as _drop_subtask_ticket_refs,
    drop_unlinked_refs as _drop_unlinked_refs,
    drop_unverified_refs as _drop_unverified_refs,
    html_tag_end as _html_tag_end,
    map_visible_body_text as _map_visible_body_text,
    merge_refs as _merge_refs,
    topic_drift as _topic_drift,
    visible_body_text as _visible_body_text,
)
from app.agent.workflow.work_architect.change_plan import (
    ChangePlanPolicies,
    build_change_plan,
)
from app.agent.workflow.work_architect.due_dates import (
    apply_relative_due_to_single_draft as _apply_relative_due_to_single_draft,
    authoritative_explicit_due as _authoritative_explicit_due,
    explicit_due_candidates as _explicit_due_candidates,
    explicit_due_instruction_status as _explicit_due_instruction_status,
    global_exact_due_for_roots as _global_exact_due_for_roots,
    normalize_due_rationale as _normalize_due_rationale,
    relative_due as _relative_due,
...
{last_user_text(state)}{data}"""
    ...
    def apply(self, state, out):
        """Run the draft post-processing pipeline with current facade policies."""
        return apply_work_architect(self, state, out, globals())
...
def _change_plan(state, out, items, qs):
    """Delegate existing-ticket planning while preserving facade policy hooks."""
    policies = ChangePlanPolicies(
        priority_map=_PRI,
        bulk_comment_preview=_bulk_comment_preview,
        comment_forbidden=_comment_forbidden,
        comment_input_missing=_comment_input_missing,
        current_request_boundary_text=_current_request_boundary_text,
        explicit_meeting_update_fields=_explicit_meeting_update_fields,
        is_change_action=_is_change_action,
        materialize_requested_update_effects=materialize_requested_update_effects,
        meeting_decision_comment=_meeting_decision_comment,
        meeting_unchanged_fields=_meeting_unchanged_fields,
        normalize_priority=_normalize_priority,
        relative_due=_relative_due,
        requested_assignee_name=_requested_assignee_name,
        same_field_value=_same_field_value,
        ticket_exists=_ticket_exists,
        typed_target_keys=_typed_target_keys,
        work_action=_work_action,
    )
    return build_change_plan(state, out, items, qs, policies)
