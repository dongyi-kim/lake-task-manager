...
- 정식 릴리즈 태그는 `vYYYY.MM.DD` 형식. 같은 날짜에 `main` 병합이 추가로 릴리즈되면
  병합 순서대로 `.1`, `.2`, …를 붙인다. 예: `v2026.08.13`, `v2026.08.13.1`.
- 릴리즈 노트는 태그의 하위 버전을 제거한 `vYYYY.MM.DD` 날짜 단위로만 구분한다. `.1`, `.2`, …
  태그를 추가할 때 새 항목을 만들지 않고 같은 날짜의 기존 항목에 변경 내용을 통합한다.
- `app/static/lib/releaseNotes.js`의 각 항목은 `version: "vYYYY.MM.DD"`만 사용한다. 버전명 자체가
  날짜이므로 별도 `date` 필드를 두거나 화면에 날짜를 중복 표시하지 않는다.
- 새 태그를 만들기 전에 해당 날짜의 릴리즈 노트를 추가하거나 갱신한다. 코드 병합·배포만 하고
  릴리즈 노트를 생략하지 않는다.
- 노트에는 사용자가 느낄 변화, 해결된 문제, 필요한 조치만 짧게 작성한다. 내부 구현 세부사항과
  커밋 나열은 넣지 않으며, 내부 변경뿐인 릴리즈도 안정성·운영 영향으로 설명한다.
- 메인 화면은 `RELEASES`의 최신 5개 날짜만 표시한다. 새 날짜 항목은 항상 맨 위에 추가하고
  `version` 날짜 내림차순을 유지한다.
...
| DL-9019~9031 | **'내 Task' 화면** — 담당 조합·Epic 없음·마감 초과/오늘/없음·완료 제외 (아래) |
| DL-9036 | **첨부 파일 종류** — 확장자 아이콘·언어 색(코드/문서/표/압축/미디어 21종). 본문·코멘트에도 `[^파일명]` 으로 넣어 **첨부 목록 칩**(프론트 렌더)과 **본문 속 뱃지**(서버 HTML)를 함께 검증한다 — 렌더 경로가 달라 한쪽만 맞은 적이 있다 |

**'내 Task' 픽스처(DL-9019~9031)** — 이 화면의 어려움은 *담당 조합*이라 조합을 다 심어 뒀다.
세션 사용자(`/myself`)는 `world.World.ME`(=`test.ui01`)로, `fakebridge` 가 jira820 `current_user` 에 넣는다.

| 키 | 검증 포인트 |
|----|------------|
| DL-9019 | 전용 Epic(UI 회귀 Epic 과 분리 — 섞으면 양쪽 검증이 흐려진다) |
| DL-9020 (+9021~9024) | **내 Task + 하위에 내 것 2 / 동료 것 2** — 동료 집계 칩·롤업 |
| DL-9025 (+9026~9027) | **남의 Task + 내가 Sub 만 담당** — 흐린 부모 헤더·상위 진척 캡슐 |
| DL-9028 | Epic 없는 내 Task + **마감 초과(D+1)** — 목록 최상단 |
| DL-9029 | Epic 없는 내 Task + **오늘 마감** |
| DL-9030 | **마감 없는** 내 Task — 정렬에서 맨 뒤 버킷 |
| DL-9031 | **완료된** 내 Task — 기본 목록에서 제외('완료 포함' 토글로만 노출) |
