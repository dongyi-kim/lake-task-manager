...
- 새 태그를 만들기 전에 `app/static/lib/releaseNotes.js` 맨 위에 태그와 정확히 일치하는 `version`의
  릴리즈 노트를 추가한다. 코드 병합·배포만 하고 릴리즈 노트를 생략하지 않는다.
- 노트에는 사용자가 느낄 변화, 해결된 문제, 필요한 조치만 짧게 작성한다. 내부 구현 세부사항과
  커밋 나열은 넣지 않으며, 내부 변경뿐인 릴리즈도 안정성·운영 영향으로 설명한다.
- 메인 화면은 `RELEASES`의 최신 5개 버전만 표시한다. 새 항목은 항상 맨 위에 추가하고
  `version` 내림차순과 실제 릴리즈 태그 순서를 일치시킨다.
